...
const getOrCreateWalkInCustomer = async (userId) => {
  let walkIn = await Customer.findOne({ userId, customerType: "SERVICE_ONLY", name: "__WALK_IN__" });
  ...
  try {
    ...
    for (const item of cart) {
      if (!mongoose.Types.ObjectId.isValid(item.inventoryItemId)) {
        return res.status(400).json({ success: false, message: `Invalid inventory item: ${item.name}` });
      }

      const invItem = await InventoryItem.findOne({ _id: item.inventoryItemId, userId });
      if (!invItem) {
    ...
    console.error(error);
    return res.status(500).json({ success: false, message: "Failed to record parts sale" });
  }
};
